...
function initMap() {
    ...
    google.maps.event.addListener(
        map,
        "bounds_changed",() => {
            fetch(`http://api.openweathermap.org/data/2.5/box/city?appid=01678c79c23052bebf6673eec49d6a8d&bbox=${map.getMapBbox()}`)
            .then(function(response) {
                response.json().then(function(data) {
                  if (data.list) {
                      if (!downloadedCities.includes(data.list[0].name)) {
                        fetch(`http://api.openweathermap.org/data/2.5/weather?q=${data.list[0].name}&appid=01678c79c23052bebf6673eec49d6a8d`)
                        .then(function(res) {
